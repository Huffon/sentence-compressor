...
def train(config):
    model_config = f"--arch transformer --share-all-embeddings " \
                 ...
                 f"--max-epoch {config.max_epoch} --warmup-updates 4000 --warmup-init-lr '1e-07' --max-tokens 4000 " \
                 f"--adam-betas '(0.9, 0.98)' --save-interval-updates 5000 "
    train_prompt = f"fairseq-train {config.databin_path} {model_config} --save-dir {config.ckpt_dir} "
